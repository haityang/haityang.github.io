...
s ：取代，可以直接进行取代的工作哩！通常这个 s 的动作可以搭配正规表示法！例如 1,20 s/old/new/g 就是啦！
y : 替换单个字符
...
\<     Beginning of a word
\>     End of a word
/⟨expression⟩/   两斜杠间的正则表达式

$ sed -e 's/\<lead\>/gold/'     //替换lead用gold
$ echo 'wéîrd fíle näme' | sed -e 'y/ äéîí/_?/'     //w??rd_f?le_n?me
$ cat -E sed-script
...
```
awk -F, '{print $1}' 1.log > r.txt   从文件到文件
