...
163.com
==========================================
用dig程序查询MX记录:http://www.isc.org/software/bind
dig 163.com mx 
...
==========================================
3. MacPorts =======//can down unix software automatically
cd MacPorts-1.9.2
...
4. Fink source ====//can down unix software automatically
./bootstrap

5.convert rose.jpg rose.png
...
6.tcpdump -i eth0 src host 192.168.0.5//在作为网关的主机上，想知道本地网络中IP地址为192.168.0.5的主机现在与外界通信的情况

7.nmap -sP 192.168.0.1//这台主机是否下线了？用Nmap探测试试看
8.nmap -sP 192.168.0.0/24 //24表明目标是一个网络而非单个主机,知道目的网络究竟有多少主机处于运行状态：

9.nmap 192.168.0.5//想要了解该主机的详细信息，可以执行
10.nmap -O 192.168.0.5  //强大的Nmap本身就可以依据TCP/IP的指纹特征猜测对方使用的操作系统。我们可以用-O来开启这一选项

11.Netstat主要用于Linux/Unix主机察看自身的网络状况，如开启的端口、在为哪些用户服务以及服务的状态等等
12.netstat -r    //Netstat还可以方便地代替route命令显示当前核心路由表
13. netstat -i       //它还可以代替ifconfig显示网络接口状态
14. telnet 192.168.1.138 8000           //telnet登录到某台主机
15. sudo tcpdump -v -i en0 -A  port 80  //显示http信息
